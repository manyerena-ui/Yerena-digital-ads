...
def get_asset_urls(asset_type: str) -> list:
    type_to_folder = {
        "owner": "owner",
...
def submit_text_to_image(prompt: str, aspect_ratio: str, resolution: str):
    payload = {
        "prompt": prompt,
    ...
    resp = requests.post(f"{FAL_BASE}/fal-ai/nano-banana-2", headers=HEADERS, json=payload, timeout=30)
...
def submit_image_edit(prompt: str, image_urls: list, aspect_ratio: str, resolution: str):
    payload = {
        "prompt": prompt,
    ...
    resp = requests.post(f"{FAL_BASE}/fal-ai/nano-banana-2/edit", headers=HEADERS, json=payload, timeout=30)
    if resp.status_code in (200, 201):
        return resp.json().get("request_id")
    else:
        print(f"  ✗ Submit failed: {resp.status_code} {resp.text}")
...
def poll_for_result(request_id: str):
    ...
    while time.time() - start < MAX_WAIT:
        resp = requests.get(f"{FAL_BASE}/fal-ai/nano-banana-2/requests/{request_id}/status", headers=HEADERS, timeout=15)
        if resp.status_code == 200:
            status_data = resp.json()
            status = status_data.get("status")
            if status == "COMPLETED":
                result_resp = requests.get(f"{FAL_BASE}/fal-ai/nano-banana-2/requests/{request_id}", headers=HEADERS, timeout=15)
...
def main():
    ...
    for i, prompt_data in enumerate(prompts_to_run):
        ...
        if needs_refs:
            print(f"  Uploading reference images (type: {asset_type})...")
            image_urls = get_asset_urls(asset_type)
            if image_urls:
                print(f"  Submitting to /edit endpoint with {len(image_urls)} reference image(s)...")
                request_id = submit_image_edit(prompt_text, image_urls, aspect_ratio, resolution)
            else:
                print(f"  ⚠ No reference images found — falling back to text-to-image")
                request_id = submit_text_to_image(prompt_text, aspect_ratio, resolution)
        else:
        ...
        print(f"  Waiting for result (request_id: {request_id[:12]}...)...")
        ...
        if result:
            ...
            print(f"  ✗ No result for template #{t_num}\n")
