    ...
    for i, prompt_data in enumerate(prompts_to_run):
        ...
        if needs_refs:
            ...
            if image_urls:
                ...
                request_id = submit_image_edit(prompt_text, image_urls, aspect_ratio, resolution, num_images)
            else:
                print(f"  ⚠ No reference images found — falling back to text-to-image")
                request_id = submit_text_to_image(prompt_text, aspect_ratio, resolution, num_images)
        else:
            print(f"  Submitting to text-to-image endpoint...")
            request_id = submit_text_to_image(prompt_text, aspect_ratio, resolution, num_images)

        if request_id == "BALANCE_EXHAUSTED":
